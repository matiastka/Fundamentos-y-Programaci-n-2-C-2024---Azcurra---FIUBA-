...
void mostrar_estructura_dinamica(int *puntero, int cant_elementos){
    int i;

    for (i = 0; i < cant_elementos; i++){
        printf("El vector en memoria dinamica en la posicion %i es: %i\n", i, puntero[i]);
    }
}
...
void main(){
    ...
    if (puntero != NULL){
        mostrar_estructura_dinamica(puntero, cant_elementos);
        free(puntero);
    }
    else{
